...
SELECT posts.id, posts.created_at, posts.updated_at, published_at, title, url, description, posts.feed_id, feed_follows.id, feed_follows.created_at, feed_follows.updated_at, user_id, feed_follows.feed_id FROM posts JOIN feed_follows ON posts.feed_id = feed_follows.feed_id WHERE feed_follows.user_id = $1 ORDER BY posts.published_at DESC LIMIT $2
`
